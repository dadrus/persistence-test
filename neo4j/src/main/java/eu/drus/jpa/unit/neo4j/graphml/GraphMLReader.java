    ...
    private static class GraphMLHandler<V, E> extends DefaultHandler {
        ...
        public void startElement(final String uri, final String localName, final String qName, final Attributes attributes)
            ...
            case KEY:
                ...
                currentKey = new Key(keyId, keyAttrName, AttributeType.fromString(keyAttrType), toKeyTarget(keyFor));
                break;
            case DEFAULT:
                insideDefault = true;
                break;
        ...
        private void addEdge(final GraphElement p) throws SAXException {
            ...
            // create attributes
            final Map<String, Object> attributes = getAttributes(p, edgeValidKeys);
        ...
        private void setEdgeWeightIfRequired(final E e, final Map<String, Object> attributes) {
            // special handling for weighted graphs
            if (g instanceof WeightedGraph<?, ?> && attributes.containsKey(EDGE_WEIGHT_ATTRIBUTE_NAME)) {
                try {
                    ((WeightedGraph<V, E>) g).setEdgeWeight(e, (Float) attributes.get(EDGE_WEIGHT_ATTRIBUTE_NAME));
        ...
        private Map<String, Object> getAttributes(final GraphElement ge, final Map<String, Key> keyReferences) {
            final Map<String, Object> attributes = new HashMap<>();

            for (final Key keyReference : keyReferences.values()) {
                if (ge.attributes.containsKey(keyReference.id)) {
                    attributes.put(keyReference.attributeName, keyReference.attributeType.convert(ge.attributes.get(keyReference.id)));
                } else if (keyReference.defaultValue != null) {
                    attributes.put(keyReference.attributeName, keyReference.attributeType.convert(keyReference.defaultValue));
                }
            }
